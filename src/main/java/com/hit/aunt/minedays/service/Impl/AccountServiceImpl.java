...
import com.hit.aunt.minedays.entity.Account;
import com.hit.aunt.minedays.service.AccountService;
...
public class AccountServiceImpl implements AccountService {
    ...
    public int add(Account x) {
        //执行写入
        int row = jdbcTemplate.update("INSERT INTO tab_account VALUES (?,?,?,?,?,?)",  new PreparedStatementSetter(){
            //映射数据
            @Override
            public void setValues(PreparedStatement ps) throws SQLException {
                ps.setLong(1, x.getAccountID());
                ps.setString(2, x.getEmail());
                ps.setString(3,x.getPassword());
                ps.setDate(4,new Date(x.getCreateTime().getTime()));
                ps.setDate(5,new Date(x.getLastEditTime().getTime()));
                ps.setLong(6,x.getPerson().getUserId());
            }
        });
        //返回结果
        return row;
    ...
    public List<Account> emlquery(String emails)  {
        ...
        List<Account> list = jdbcTemplate.query(sql, new RowMapper<Account>() {
            ...
            public Account mapRow(ResultSet rs, int rowNum) throws SQLException {
                ...
                account.setPerson(personServices.idquery(rs.getLong("userID")).get(0));
                return account;
            }
